...
import { JournalCleanTools } from './tools/journal-clean.js';
...
async function processMapGenerationInBackend(
  ...
  try {
    ...
    try {
      uploadResult = await foundryClient.query('ninjos-foundry-mcp.upload-generated-map', {
        filename: filename,
...
async function startBackend(): Promise<void> {
  ...
  const journalCleanTools = new JournalCleanTools({ foundryClient, logger });
  ...
  const allTools = [
    ...
    ...journalCleanTools.getToolDefinitions(),
          ...
          if (msg.method === 'call_tool') {
            ...
            try {
              ...
              switch (name) {
                ...
                case 'journal-create':
                  result = await journalCleanTools.handleCreate(args);

                  break;

                case 'journal-set-page':
                  result = await journalCleanTools.handleSetPage(args);

                  break;

                case 'journal-add-page':
                  result = await journalCleanTools.handleAddPage(args);

                  break;

                case 'journal-delete-page':
                  result = await journalCleanTools.handleDeletePage(args);

                  break;

                case 'journal-delete':
                  result = await journalCleanTools.handleDelete(args);
